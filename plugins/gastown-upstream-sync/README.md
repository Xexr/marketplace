# Gastown Upstream Sync Plugin

Fork maintenance for gastown (steveyegge/gastown) - upstream sync, cherry-pick management, binary rebuild, formula sync, and branch cleanup.
...
- Periodic fork hygiene (branch cleanup, binary rebuild, formula sync)

## Prerequisites

- A gastown crew clone with `origin` pointing to your fork and `upstream` pointing to steveyegge/gastown
- On the `main` branch
...
11. **Formulas & Doctor** - `gt doctor --fix` to sync formulas and run checks
