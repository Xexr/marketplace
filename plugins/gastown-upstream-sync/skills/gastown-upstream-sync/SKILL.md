...
description: Use when syncing a gastown fork with upstream, updating the gt binary, or cleaning up merged PR branches. Triggers on phrases like "sync gastown upstream", "pull gastown upstream", "update gastown fork", "upgrade gastown", "clean up gastown branches".
---
...
- Working directory: a gastown crew clone (e.g. `~/gt/gastown/crew/<crew-name>`)
- Remotes: `origin` = your fork, `upstream` = steveyegge/gastown
...
> - **Fork:** `<OWNER>/<REPO>` (origin) forked from `steveyegge/gastown` (upstream)
> - **Crew name:** `<detected>`
...
collisions (invisible to git rebase), formula drift, and test regressions.
...
### 10. Sync Formulas and Run Doctor
...
This syncs embedded formulas from the repo to the town's runtime config AND runs
all doctor checks in one pass. Report any changes detected and any issues found.
